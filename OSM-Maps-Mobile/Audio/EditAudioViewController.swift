...
protocol EditAudioDelegate {
    func audioChanged(item: AudioItem)
}

class EditAudioViewController: ScrollViewController{
    ...
    var delegate: EditAudioDelegate?
    ...
    func loadScrollableSubviews() {
        ...
        audioView.setupView()
        contentView.addSubviewWithAnchors(audioView, top: contentView.topAnchor, leading: contentView.leadingAnchor, trailing: contentView.trailingAnchor)
        audioView.url = item.url
        audioView.enablePlayer()
        let header = UILabel(header: "coordinate".localize())
        contentView.addSubviewBelow(header, upperView: audioView)
    ...
    func save(){
        ...
        if newCoord != nil || newDate != nil{
            ...
            AppData.shared.save()
            delegate?.audioChanged(item: item)
        }
        self.close()
